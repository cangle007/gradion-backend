...
You will build a REST API for a lightweight Expense Report Management System. The scope is intentionally limited to fit a single working day. Resist the urge to over-build.
...
| `Expense Item` | id, report_id (FK), amount, currency, category, merchant_name, transaction_date | Cascade delete with Report |
...
No frontend is required. A clean REST API with JSON responses is sufficient.
...
- A `docker-compose.yml` to spin up all dependencies (database, etc.).
...
These are not expected and will not negatively affect your evaluation if absent. They are here if you finish early and want to show more depth. **We fully expect strong candidates to use AI tools to move fast on these — that's the point:**

- Receipt image upload (local filesystem or MinIO mock is fine).
- A simple AI-powered endpoint — e.g., suggest expense category from merchant name via an LLM API.
- Background job processing for any async work.
- A basic frontend UI (React, Vue, or plain HTML — your choice). At minimum: login, expense report list, submit/view report flow. Styling is secondary; usability and clean component structure matter more.
...
5. Expose admin endpoints (list, approve, reject).
6. Write `DECISIONS.md` and AI usage note. Commit AI artifacts. Polish `README`.
